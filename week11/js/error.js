export class Errors {
  ...
  export async function makeRequest(
    ...
    // processing the response as JSON before we check the status
    const data = await response.json();
  
    if (!response.ok) {
      // server will send a 500 server error if the token expires...or a 401 if we are not authorized, 
      // ie bad username/password combination, and a 404 if the URL we requested does not exist. All of these would cause response.ok to be false
